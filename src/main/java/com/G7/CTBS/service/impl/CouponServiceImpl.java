...
public class CouponServiceImpl implements CouponService {
    ...
    private String resolveStatus(Coupon coupon) {
        // 1. VÁ LỖI MÚI GIỜ: Lấy chính xác ngày hiện tại ở Việt Nam
        LocalDate today = LocalDate.now(java.time.ZoneId.of("Asia/Ho_Chi_Minh"));

        // 2. VÁ LỖI NULL: Kiểm tra xem đã quá ngày hết hạn chưa (Ưu tiên cao nhất)
        if (coupon.getExpiryDate() != null && today.isAfter(coupon.getExpiryDate())) {
            return "EXPIRED";
        }

        // 3. BỔ SUNG: Kiểm tra xem đã hết lượt sử dụng chưa
        if (coupon.getMaxUsage() != null && coupon.getUsedCount() != null && coupon.getUsedCount() >= coupon.getMaxUsage()) {
            return "EXHAUSTED"; // Báo trạng thái Hết lượt
        }

        // 4. Kiểm tra nút gạt kích hoạt (Active)
        if (coupon.getActive() == null || !coupon.getActive()) {
            return "INACTIVE";
        }

        // Vượt qua 3 vòng kiểm duyệt trên thì mới thực sự là ACTIVE
        return "ACTIVE";
    }
    ...
    public Coupon validateCoupon(String code, Double currentAmount) {
        ...
        // VÁ LỖI MÚI GIỜ: Đồng bộ giờ Việt Nam
        LocalDate today = LocalDate.now(java.time.ZoneId.of("Asia/Ho_Chi_Minh"));
        ...
        // 2. Check expiry date (Đã áp dụng chuẩn giờ Việt Nam)
