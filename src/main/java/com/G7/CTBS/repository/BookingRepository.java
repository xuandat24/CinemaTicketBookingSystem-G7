...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    List<Booking> findByStatusAndCreateTimeBetween(String status, LocalDateTime start, LocalDateTime end);
    
    @Query("SELECT SUM(b.finalPrice) FROM Booking b WHERE b.status = 'SUCCESS' AND b.createTime BETWEEN :start AND :end")
    Double getTotalRevenue(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);
    
    @Query("SELECT COUNT(bs) FROM Booking b JOIN b.bookingSeats bs WHERE b.status = 'SUCCESS' AND b.createTime BETWEEN :start AND :end")
    ...
    @Query("SELECT new com.G7.CTBS.dto.MovieRevenueDTO(m.title, SUM(b.finalPrice)) " +
            ...
            "WHERE b.status = 'SUCCESS' AND b.createTime BETWEEN :start AND :end " +
    ...
    @Query("SELECT COUNT(bs) FROM Booking b JOIN b.bookingSeats bs " +
            "WHERE b.showtime.showtimeId = :showtimeId AND b.status = 'SUCCESS'")
