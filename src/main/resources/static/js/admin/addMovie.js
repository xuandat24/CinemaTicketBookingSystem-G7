...
if (formEl) {
    ...
    if (formEl) {
        formEl.addEventListener('submit', async (e) => {
            ...
            // 1. INLINE ERROR (at Form) if empty
            ...
            if (selectedCategoryIds.length === 0 && !genres) {
                errorEle.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> ERROR: Please select at least 1 category for the movie!';
                errorEle.style.display = 'block';
                return;
            }
            ...
            if (!bannerFile && !bannerUrl) {
                errorEle.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> ERROR: Please upload a Banner image or provide a TMDB link!';
                errorEle.style.display = 'block';
                return;
            }
            if (!posterFile && !posterUrl) {
                errorEle.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> ERROR: Please upload a Poster image or provide a TMDB link!';
                errorEle.style.display = 'block';
                return;
            }
            ...
            try {
                ...
                // 2. SHOW RESULT USING POP-UP TOAST
                if (response.ok) {
                    showToast("Movie added successfully!", "success");
                    setTimeout(() => { window.location.href = '/admin/movies/search'; }, 1500);
                } else {
                    showToast(data.message || "Failed to save movie! Please try again.", "error");
