            ...
            <div class="carousel-item ${isActive}">
                <a href="/detail?id=${movie.movieId}" class="banner-link-wrapper">
                    ...
                    <div class="carousel-caption">
                        ...
                        <div class="hero-ticket-btn"><i class="fa fa-info-circle fs-5 me-2"></i> GET DETAIL</div>
                    </div>
                </a>
            </div>
        `;
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        // Logic for Banner
        let bannerMovies = nowPlayingMovies.slice(0, 5);
        if (bannerMovies.length === 0) bannerMovies = comingSoonMovies.slice(0, 5);
...
function renderNowShowing(movies) {
    const container = document.getElementById('nowShowingSlider'); // Check this ID in index.html
    if (!container) return;

    if (movies.length === 0) {
        container.closest('.movie-slider-wrapper').innerHTML = `
            <div class="text-center py-5 w-100">
                <h5 class="text-muted">No movies are currently showing.</h5>
            </div>`;
        return;
    }
                ...
                <div class="movie-card mb-0">
                    <div class="movie-poster-wrapper">
                        ...
                        <div class="movie-overlay">
                            <a href="/detail?id=${movie.movieId}" class="btn-book-now mt-3">Get Detail</a>
                        </div>
                    </div>
...
function renderComingSoon(movies) {
    // QUAN TRỌNG: Kiểm tra ID này trong index.html của bạn.
    // Nếu là "comingSoonSlider1" thì phải sửa ở đây cho đúng.
    ...
    if (movies.length === 0) {
        container.innerHTML = `
            <div class="text-center py-5 w-100">
                <h5 class="text-white-50">No upcoming movies at the moment.</h5>
            </div>`;
        return;
    }
            ...
            <div class="coming-soon-slider-item" style="min-width: 320px; margin-right: 15px;">
                <div class="coming-soon-card position-relative">
                    ...
                    <div class="position-absolute bottom-0 start-0 p-3 w-100">
                        <span class="badge bg-warning text-dark mb-2">${movie.releaseDate || 'Coming Soon'}</span>
                        <h4 class="text-white text-truncate">${movie.title}</h4>
                        <a href="/detail?id=${movie.movieId}" class="btn btn-sm btn-outline-light">GET DETAIL</a>
                    </div>
                </div>
            </div>`;
    });
